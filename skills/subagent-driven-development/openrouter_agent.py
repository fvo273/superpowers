...
def grep_files(pattern: str, path: str = ".", file_pattern: str = "*") -> str:
    ...
    # Pure-Python fallback for environments without rg or grep (e.g. Windows)
    import re
    try:
        regex = re.compile(pattern)
        file_glob = glob_module.glob(str(Path(path) / "**" / file_pattern), recursive=True)
        lines = []
        for filepath in sorted(file_glob):
            if not Path(filepath).is_file():
                continue
            try:
                for i, line in enumerate(Path(filepath).read_text(encoding="utf-8", errors="replace").splitlines(), 1):
